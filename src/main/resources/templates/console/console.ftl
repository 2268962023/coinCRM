            ...
            <div class="layui-card">
                <div class="layui-card-body">
                    <div class="layui-tab custom-tab layui-tab-brief" lay-filter="docDemoTabBrief">
                        <div id="main" style="background-color:#ffffff;min-height:400px;padding: 10px"></div>
                    </div>
                </div>
            </div>
            <div class="layui-card">
                <div class="layui-card-body">
                    <div id="turnover_order" style="background-color:#ffffff;min-height:400px;padding: 10px"></div>
                </div>
            </div>
        </div>
...
<script type="text/javascript">
    layui.use(['layer', 'echarts', 'element', 'count', 'topBar'], function () {
        ...
        var echartsRecords = document.getElementById('main');
        var myChart = echarts.init(echartsRecords);
        $.post("/echarts/authStatus",function (data) {
            var option = {
                tooltip: {
                    trigger: 'item',
                    formatter: "{a} <br/>{b} : {c} "
        ...
        var chartDom = document.getElementById("turnover_order");
        var myCharts = echarts.init(chartDom);
        var options;

        $.post("echarts/countTurnoverOrder",function (datas) {

            const dataCount = datas;
         ...
         * 系统公告
         */
        $.get("echarts/selectNotice",function (data) {
            var noticelist = document.getElementById("list");
            for (var notice in data) {
                 let li = window.document.createElement("li");
                 li.innerHTML="<li class='list-item'> <span class='title'>"+data[notice].title+"</span> <span class='footer'>"+layui.util.toDateString(data[notice].created)+"</span> </li>"
